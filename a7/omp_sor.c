...
int main(int argc, char *argv[])
{
    ...
    do {
        ...
        #pragma omp parallel for shared(mP,x_temp,x) private(q,i,j)
        for (q=0;q<mP*mP;q++){
            // only one for loop for better parallelization with openMP - 
            //local_squaredEuclideanError ist immer von der vorherigen Iteration abhängig => also nicht einfach parallelisierbar
            //calculating i,j (since x has a ghostlayer)
            i = q / mP;
            j = q % mP;
        ...
        for (int color=0;color<2;color++){
            ...
            // Durch die Unabhängigkeit der Vektoreinträge von ihren Nachbarn können (red black sor) können wir diesen for loop parallelisieren
            #pragma omp parallel for shared(mP,x) private(q,i,j)
            for (q=0;q<mP*mP;q++){
                // only one for loop for better parallelization with openMP - 
                //local_squaredEuclideanError ist immer von der vorherigen Iteration abhängig => also nicht einfach parallelisierbar
                //calculating i,j (since x has a ghostlayer)
        ...
        for (int q=0;q<mP*mP;q++){
            // only one for loop for better parallelization with openMP - 
            //local_squaredEuclideanError ist immer von der vorherigen Iteration abhängig => also nicht einfach parallelisierbar
            //calculating i,j (since x has a ghostlayer)
            i = q / mP;
            j = q % mP;
            //x_temp is x from previous iteration
